...
const FlightForm = ({ onFlightAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (listaPersonas.length === 0) {
      return Swal.fire(
        "Error",
        "Debe agregar al menos una persona al manifiesto",
        "error"
      );
    }
    ...
    try {
      ...
      const dataFinal = {
        ...
        procedencia: formData.procedencia || "N/A",
        destino: formData.destino || "N/A",
        // Forzamos que los datos del oficial no vayan vacíos
        nombreOficial: formData.nombreOficial.toUpperCase(),
      ...
      Swal.fire({
        ...
        } de ${formData.matricula.toUpperCase()} guardado correctamente.`,
        background: "#0f172a",
        color: "#fff",
      ...
      const errorMsg =
        ...
        "Faltan completar campos obligatorios o el formato es incorrecto.";
      Swal.fire({
        icon: "error",
        title: "Error 400 - Datos Inválidos",
        text: errorMsg,
        background: "#0f172a",
        color: "#fff",
      });
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1">
            <label className="text-[10px] text-slate-500 font-bold ml-1 uppercase">
              Fecha
            </label>
            <input
              type="date"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input
            type="text"
            name="tipoAeronave"
            placeholder="TIPO AERONAVE (Ej: C172)"
          ...
          <div className="grid grid-cols-1 md:grid-cols-7 gap-3 items-end">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="p-2 bg-slate-900 text-white rounded-lg border border-slate-700 font-mono outline-none text-sm w-full"
              />
            </div>
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              ...
              <input
                ...
                onChange={handlePersonaChange}
                className="p-2 bg-slate-900 text-white rounded-lg border border-slate-700 uppercase outline-none text-sm w-full"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-[9px] text-amber-500 font-bold uppercase ml-1">
                Mano
              </label>
              <input
                type="number"
                ...
                onChange={handlePersonaChange}
                className="p-2 bg-slate-900 text-amber-500 rounded-lg border border-slate-700 outline-none text-sm text-center font-bold"
                min="0"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-[9px] text-blue-400 font-bold uppercase ml-1">
                Bodega
              </label>
              <input
                type="number"
                ...
                onChange={handlePersonaChange}
                className="p-2 bg-slate-900 text-blue-400 rounded-lg border border-slate-700 outline-none text-sm text-center font-bold"
                min="0"
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={handlePersonaChange}
                className="p-2 bg-slate-900 text-blue-400 font-bold rounded-lg border border-slate-700 outline-none text-sm"
              >
            ...
            <button
              ...
              className="h-9.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg flex items-center justify-center gap-2 font-black text-[10px] transition-all uppercase px-4"
            >
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${
                        p.tripPax === "T" ? "bg-blue-500" : "bg-purple-500"
                      }`}
                    ></div>
                  ...
                  <div className="flex gap-4 ml-5">
                    <span className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1">
                      <Briefcase size={10} /> Mano: {p.equipajeMano}
                    </span>
                    <span className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1">
                      <Package size={10} /> Bodega: {p.equipajeBodega}
                    </span>
                  </div>
                </div>
                <button
        ...
        {/* Oficial de Turno */}
        <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 space-y-4">
          <h3 className="text-slate-500 font-black text-[10px] uppercase tracking-[0.2em]">
            Oficial de Turno
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <select
              ...
              onChange={handleChange}
              className="p-3 bg-slate-900 text-white border border-slate-700 rounded-lg font-bold outline-none"
            >
            ...
            <input
              ...
              onChange={handleChange}
              className="p-3 bg-slate-900 text-white border border-slate-700 rounded-lg uppercase outline-none"
              required
            />
            <input
              type="text"
              ...
              onChange={handleChange}
              className="p-3 bg-slate-900 text-white border border-slate-700 rounded-lg outline-none"
              required
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          className={`w-full py-4 rounded-xl font-black text-white transition-all shadow-2xl flex items-center justify-center gap-3 ${
            formData.tipoMovimiento === "ARRIBO"
